...
- `rerank` (bool, optional): Enable cross-encoder reranking for improved relevance (default: false)
- `rerank_candidates` (int, optional): Number of candidates to fetch before reranking (default: 50)
...
Reranking is a **two-stage retrieval** process that significantly improves result relevance:

1. **Stage 1 - Fast Vector Search** (bi-encoder):
   - Uses Vertex AI embeddings for initial candidate retrieval
   - Fast cosine similarity search across all documents
   - Fetches `rerank_candidates` results (default: 50)

2. **Stage 2 - Precision Reranking**:
   - Uses LLM (Gemini, default) or cross-encoder to score relevance
   - More accurate than vector similarity (Google's recommended approach)
   - Returns top `top_k` results with `rerank_score` field

**Supported reranking methods:**
- **Gemini LLM** (default, recommended): Best quality, uses Google AI
- **Local cross-encoder**: Fast, offline-capable, no API costs
- **Cohere API**: High quality, commercial

See [docs/reranking.md](docs/reranking.md) for detailed comparison and configuration guide.

**When to use reranking:**
- ✅ **Quality-critical queries:** Search where precision matters (legal docs, medical, financial)
- ✅ **Ambiguous queries:** When semantic meaning is nuanced
- ✅ **Final results for users:** When showing top 3-5 results to end users
- ❌ **Batch processing:** Too slow for high-volume/low-latency scenarios
- ❌ **Simple keyword matching:** Vector search alone is usually sufficient

**Performance:**
- **Without reranking:** ~100-200ms per query
- **With Gemini reranking (20 docs):** ~7-8s (async batching: 10 parallel batches × 2 docs)
- **With Gemini reranking (5 docs):** ~500-800ms (1-3 parallel batches)
- **With local cross-encoder:** ~100ms per query (local model inference)
- **Trade-off:** Slower, but better semantic understanding and reasoning

**Cost:**
- **Gemini reranking:** ~$0.001 per query (API tokens)
- **Local reranking:** ~$0.026 per 1K queries (compute only)
- **Vector search only:** Same embedding costs, no reranking overhead
...
# Enable Gemini LLM reranking (recommended)
RERANKER_ENABLED=true
RERANKER_TYPE=gemini
RERANKER_MODEL=gemini-2.0-flash-exp

# Or use local cross-encoder (fast, offline)
# RERANKER_ENABLED=true
# RERANKER_TYPE=local
...
curl -X POST http://localhost:8080/v1/query \
  -H "Content-Type: application/json" \
  -d '{
    "query": "best practices for Python exception handling",
    "top_k": 5,
    "rerank": true,
...
**Response with reranking** includes `rerank_score` and `rerank_reasoning` fields:
```json
{
  "query": "best practices for Python exception handling",
  "results": [
    {
      "chunk_text": "...",
      "similarity": 0.82,
      "rerank_score": 0.95,  // <-- Normalized 0-1 score (higher = better relevance)
      "rerank_reasoning": "Directly addresses exception handling best practices with code examples",  // <-- LLM explanation
...
**Note:** `rerank_reasoning` is only available with Gemini reranking (not local cross-encoder or Cohere).
