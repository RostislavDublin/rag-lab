...
def save_env_file(config: dict, connection_name: str, sa_email: str):
    """Check if .env exists, print instructions if not."""
    env_path = Path(__file__).parent.parent / '.env'
    
    if env_path.exists():
        print_success(f".env file already exists")
        print_info("To update secrets in Cloud Run: deployment/upload-secrets.sh")
    else:
        print_warn(".env file NOT found")
        print_info("")
        print_info("Next steps:")
        print_info("  1. Copy .env.example to .env: cp .env.example .env")
        print_info("  2. Edit .env and set these infrastructure values:")
        print_info(f"     DATABASE_URL=postgresql+asyncpg://{config['DB_USER']}:{config['DB_PASSWORD']}@/{config['DB_NAME']}?host=/cloudsql/{connection_name}")
        print_info(f"     GCP_PROJECT_ID={config['GCP_PROJECT_ID']}")
        print_info(f"     GCP_REGION={config['GCP_REGION']}")
        print_info(f"     GCS_BUCKET={config['GCS_BUCKET']}")
        print_info(f"     CLOUD_SQL_CONNECTION_NAME={connection_name}")
        print_info(f"     SERVICE_ACCOUNT_EMAIL={sa_email}")
        print_info("  3. Configure ALL other settings (JWT, reranking, LLM, logging)")
        print_info("  4. Upload to Secret Manager: deployment/upload-secrets.sh")
        print_info("")
