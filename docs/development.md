...
### 2. `.env` (Cloud Run Secret)

**Purpose:** Production runtime configuration in Secret Manager  
**Storage:** Secret Manager secret `raglab-config` (not a file!)  
**Database:** Cloud SQL via Unix socket  
**Auth:** Service account from Cloud Run metadata  
**Created by:** `deployment/setup-infrastructure.sh`  
**Template:** [.env.example](../.env.example)

**Use when:**
- Deploying to Cloud Run (mounted as `/secrets/raglab-config/.env`)
- Running in GCP production environment
- Service account has all necessary permissions

**Note:** This is NOT a file in the repository. It's created as a Secret Manager secret during infrastructure setup.
...
| `.env` (Secret Manager) | Production runtime | App config + Cloud SQL socket | Deployed service in Cloud Run |
